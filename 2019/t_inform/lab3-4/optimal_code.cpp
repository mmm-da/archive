...
void Alphabet::print_codes(){
	...
	cout<<"Средняя длина кода: "<<this->avg_len()<<endl<<endl;
};
...
string Alphabet::encode_text(string str){
	...
	unsigned addition_symbols = block_size - (str.length()%block_size);
		...
		cout<<"Первый байт закодированного сообщения, содержит размер блока - "<<block_size<<endl;
		cout<<"Второй байт закодированного сообщения, содержащий кол-во добавленных символов - "<<addition_symbols<<endl;
	...
	for(unsigned i=0;i<addition_symbols;i++){
		...
		cout<<"Дописываем в последний блок "<<arr[0].get_symb()[0]<<endl;
	}

	//Вычисляем размер текста в блоках.
	unsigned str_block_count=str.length()/block_size;
	
	cout<<"Сообщение после дописывания символов в последний блок - ";
	for(int i=0;i<str_block_count;i++){
			cout<<str.substr(i*block_size,block_size)<<" ";
	};
	cout<<endl;

	cout<<"Закодированное сообщение - "<<num_to_binstr(block_size)<<" "<<num_to_binstr(addition_symbols)<<" ";
	for(unsigned i=0;i<str_block_count;i++){
		...
		if(flag){
			cout<<"	Неверно введено слово. Завершение программы."<<endl;
			exit(1);
		}
		else {
